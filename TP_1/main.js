...
//     console.log("Pepe no debería poder ver esa pelicula"); // ok
// if (!otro_usuario.ver(una_peli, 100))
//     console.log("Juan debería poder ver esa pelicula"); // ok
// if (!otro_usuario.viendo(una_peli))
//     console.log("Juan todavía está viendo la peli"); // ok
// if (!otro_usuario.ver(una_peli, 40))
//     console.log("Juan debería poder retomar la pelicula"); // ok
// if (otro_usuario.viendo(una_peli))
//     console.log("Juan ya terminó la peli");  // ok
// if (!otro_usuario.visto(una_peli))
//     console.log("Juan vió la peli"); //ok
// Pruebas para la serie
if (!mi_usuario.ver(una_serie, 20))
    console.log("Pepe debería poder ver esa serie"); // ok
if (mi_usuario.capituloActual(una_serie) != 0)
    console.log("Pepe está en el capitulo 0"); // ok
if (!mi_usuario.ver(una_serie, 40))
    console.log("Pepe debería poder ver esa serie"); // ok
if (mi_usuario.capituloActual(una_serie) != 1)
    console.log("Pepe está en el capitulo 1"); // ok
if (!mi_usuario.ver(una_serie, 40))
    console.log("Pepe debería poder ver esa serie"); // ok
if (mi_usuario.capituloActual(una_serie) != 1)
    console.log("a Pepe le queda un minuto del capitulo 1"); // ok
...
if (!mi_usuario.viendo(una_serie))
    console.log("Pepe todavía está viendo la serie"); // ok
if (mi_usuario.visto(una_serie))
    console.log("Pepe todavía no terminó la serie"); // ok
if (!mi_usuario.ver(una_serie, 5))
    console.log("Pepe debería poder ver esa serie"); // ok
if (mi_usuario.viendo(una_serie))
    console.log("Pepe ya dejó de ver la serie"); // ok
if (!mi_usuario.visto(una_serie))
    console.log("Pepe vió la serie"); // ok
